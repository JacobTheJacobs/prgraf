...
if sys.platform == "win32":
    try:
        ...
        # In older Python versions or environments where reconfigure is not available
        import codecs
...
def get_multiline_input(prompt_text: str = "Ask a question") -> str:
    """Get multiline input from user with Ctrl+M to submit."""
    ...
    print_formatted_text(
        HTML(
            f"<ansigreen><b>{clean_prompt}</b></ansigreen> <ansiyellow>(Press Ctrl+N to submit, Enter for new line)</ansiyellow>: "
        )
    )
...
def _initialize_services_and_agent(repo_path: str, ingestor: MemgraphIngestor) -> Any:
    ...
    directory_lister_tool = create_directory_lister_tool(repo_path)
    ...
    # Optional: initialize a reasoning supervisor for meta-planning (not yet integrated into loop)
    if getattr(settings, "ENABLE_REASONING_SUPERVISOR", False):
        try:
            _ = create_reasoning_supervisor()
            logger.info("[Supervisor] Reasoning supervisor initialized.")
        except Exception as e:
            logger.warning(f"[Supervisor] Failed to init reasoning supervisor: {e}")
    return rag_agent
