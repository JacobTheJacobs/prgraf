...
class AppConfig(BaseSettings):
    ...
    FILE_READ_MAX_RETRIES: int = 3
    FILE_READ_RETRY_DELAY_MS: int = 150

    # LLM response validation retries
    LLM_OUTPUT_RETRIES: int = 3

    # --- Generation parameter knobs (optional, per-role) ---
    # Orchestrator generation defaults
    ORCHESTRATOR_TEMPERATURE: float = 0.2
    ORCHESTRATOR_TOP_P: float = 1.0
    ORCHESTRATOR_MAX_TOKENS: int = 2000

    # Cypher generator generation defaults
    CYPHER_TEMPERATURE: float = 0.0
    CYPHER_TOP_P: float = 1.0
    CYPHER_MAX_TOKENS: int = 800

    # Reasoning supervisor toggle
    ENABLE_REASONING_SUPERVISOR: bool = True
    ...
    def active_orchestrator_model(self) -> str:
        ...
        # If no cloud API keys are set, default to local models
    ...
    # --- Accessors for generation configs ---
    def get_orchestrator_generation_config(self) -> dict[str, int | float]:
        """Return generation parameters for the orchestrator model."""
        return {
            "temperature": float(self.ORCHESTRATOR_TEMPERATURE),
            "top_p": float(self.ORCHESTRATOR_TOP_P),
            "max_tokens": int(self.ORCHESTRATOR_MAX_TOKENS),
        }

    def get_cypher_generation_config(self) -> dict[str, int | float]:
        """Return generation parameters for the Cypher generator model."""
        return {
            "temperature": float(self.CYPHER_TEMPERATURE),
            "top_p": float(self.CYPHER_TOP_P),
            "max_tokens": int(self.CYPHER_MAX_TOKENS),
        }
