...
class FileReader:
    ...
    async def read_file(self, file_path: str) -> FileReadResult:
        ...
        try:
            full_path = safe_resolve_path(self.project_root, file_path)
            if isinstance(full_path, PathResolutionError):
                logger.warning(str(full_path))
                return FileReadResult(
                    file_path=file_path, error_message=str(full_path)
                )
            ...
            # Proceed with reading as a text file with retries and encoding fallbacks
            max_retries = max(0, int(getattr(settings, "FILE_READ_MAX_RETRIES", 0)))
            delay_ms = max(0, int(getattr(settings, "FILE_READ_RETRY_DELAY_MS", 0)))
            ...
            for attempt in range(max_retries + 1):
                try:
                    try:
                        ...
                        # Fallback encodings commonly seen on Windows
                        for enc in ("utf-8-sig", "cp1252", "latin-1"):
                            try:
                                content = full_path.read_text(encoding=enc)
                                break
                            except UnicodeDecodeError:
                                content = None  # continue loop
                        if content is None:
                            raise
            ...
            error_msg = (
                f"Failed to read file after {max_retries + 1} attempt(s): {last_error}"
            )
            logger.warning(f"[FileReader] {error_msg}")
            return FileReadResult(file_path=file_path, error_message=error_msg)
