...
class TypeInferenceEngine:
    ...
    def __init__(
        ...
        # Caches and recursion guards
        self._attribute_type_cache: dict[tuple[str, str], str | None] = {}
        self._attr_in_progress: set[tuple[str, str]] = set()
        self._method_return_cache: dict[str, str | None] = {}
        self._resolving_calls: set[str] = set()
    ...
    # NEW: lightweight variant used to avoid recursion explosions while discovering self.<attr> types
    def _analyze_self_assignments_simple(
        self,
        node: Node,
        local_var_types: dict[str, str],
        module_qn: str,
        ...
        if node.type == "assignment":
            ...
            if left_node and right_node and left_node.type == "attribute":
                ...
                if left_text and left_text.decode("utf8").startswith("self."):
                    ...
                    # IMPORTANT: simple inference only (no method-call chasing)
        ...
        for child in node.children:
            self._analyze_self_assignments_simple(
                child, local_var_types, module_qn, depth + 1, max_depth
            )
    ...
    def _infer_method_call_return_type(
        ...
        if cache_key in self._method_return_cache:
            return self._method_return_cache[cache_key]

        if cache_key in self._resolving_calls:
            # Break recursion cycle
            return None

        self._resolving_calls.add(cache_key)
        try:
            # Handle chained method calls first
            if "." in method_call and self._is_method_chain(method_call):
                result = self._infer_chained_call_return_type_fixed(
                    method_call, module_qn, local_var_types
                )
                self._method_return_cache[cache_key] = result
                return result

            # Try proper AST analysis for non-chained calls
            result = self._infer_method_return_type(
                method_call, module_qn, local_var_types
            )
            self._method_return_cache[cache_key] = result
            return result
        finally:
            self._resolving_calls.discard(cache_key)
    ...
    def _infer_attribute_type(self, attribute_name: str, module_qn: str) -> str | None:
        ...
        key = (module_qn, attribute_name)
        if key in self._attribute_type_cache:
            return self._attribute_type_cache[key]
        ...
        try:
            # Try to discover assignments to self.<attr> in the current module using a lightweight scan
            try:
                for file_path, (root_node, language) in self.ast_cache.items():
                    ...
                    # Lightweight analysis: only constructor-like RHS
                    ...
                    if full_attr_name in instance_vars:
                        ...
                        self._attribute_type_cache[key] = attr_type
                        return attr_type
            ...
            result = self._find_class_in_scope(class_name, module_qn)
            self._attribute_type_cache[key] = result
            return result
        finally:
